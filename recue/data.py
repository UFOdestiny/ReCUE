...
from recue.env import DATASETS_ROOT, gsm8k_gold, extract_pred_math, verify_math, normalize_num
...
def load_aime(limit: int = 0) -> List[Dict[str, Any]]:
    """AIME 2024 + 2025 competition math (60 questions). Verifiable integer answers.

    AIME24 (math-ai/aime24, parquet) stores the gold inside a boxed solution; AIME25
    (math-ai/aime25, jsonl) stores an integer `answer`. Both are pooled into one cell.
    """
    import re as _re
    import pandas as pd
    rows = []
    # AIME24: id, problem, solution=\boxed{...}, url
    p = glob.glob(os.path.join(_snap("datasets--math-ai--aime24"), "test-*.parquet"))[0]
    df = pd.read_parquet(p)
    for _, r in df.iterrows():
        m = _re.search(r"\\boxed\{([^}]*)\}", str(r["solution"]))
        a = (m.group(1) if m else str(r["solution"])).strip()
        rows.append({"id": f"aime24-{r['id']}", "question": r["problem"],
                     "gold": normalize_num(a) or a, "gold_raw": a, "type": "math"})
    # AIME25: problem, answer (int), id
    p2 = os.path.join(_snap("datasets--math-ai--aime25"), "test.jsonl")
    for line in open(p2):
        r = json.loads(line)
        a = str(r["answer"]).strip()
        rows.append({"id": f"aime25-{r['id']}", "question": r["problem"],
                     "gold": normalize_num(a) or a, "gold_raw": a, "type": "math"})
    if limit:
        rows = rows[:limit]
    return rows
