#!/usr/bin/env bash
# Build the generation + probe matrix for ReCUE (generate -> probe -> confidence).
#
...
#
# Each line runs: recue.generate -> recue.probe -> scripts.run_probe_confidence,
...
K="${RECUE_K:-8}"
MAXLEN="${RECUE_MAX_MODEL_LEN:-14336}"
...
while IFS='|' read -r model ds lim mt think tag; do
  ...
  if [ ! -f "$ROOT/gen/${tag}.json" ]; then
    ...
    python -m recue.generate --model "$model" --dataset "$ds" --limit "$lim" --k "$K" \
